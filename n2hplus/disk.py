...
class Disk:
    ...
    mN2H    = 29.021                  # - N2H+ molecular weight
...
#    mDCO    = mCO+2.014               # - HCO molecular weight
   ...
   # Tco     = 18.                     # - CO freeze out
    TN2     = 10.                     # - N2 freeze out
    sigphot = 0.79*sc                 # - photo-dissociation column

    def __init__(self,params=[-0.2,0.04,1.,1.,1000.,10**(2.823),-36.,0.54,10**(-10.73),0.5,70.,15,18, 24.68,
                 [3.2,100],[10.,1000],-1,200.],obs=[180,131,300,170],rtg=True,vcs=True,
                 exp_temp=False,line='n2h',ring=None):
    ...
    def set_params(self,params):
        ...
        self.N2H  = params[8]               # - N2H+ gas fraction
        self.vturb = params[9]*Disk.kms     # - turbulence velocity
        self.zq0 = params[10]               # - Zq, in AU, at 150 AU
        self.tmid0 = params[11]             # - Tmid at 150 AU
        self.Tco = params[12]
        self.tatm0 = params[13]             # - Tatm at 150 AU
        self.sigbound = [params[14][0]*Disk.sc,params[14][1]*Disk.sc]
              # - upper and lower column density boundaries
        if len(params[15]) ==2:
            # - inner and outer abundance boundaries
            self.Rabund = [params[15][0]*Disk.AU,params[15][1]*Disk.AU]
        else:
            # - inner/outer ring, width of inner/outer ring
            self.Rabund=[params[15][0]*Disk.AU,params[15][1]*Disk.AU,params[15][2]*Disk.AU,params[15][3]*Disk.AU,params[15][4]*Disk.AU,params[15][5]*Disk.AU]
        self.handed = params[16]            #
        self.Rbreak = params[17] #Transition radius for temperature profile
    ...
    def set_structure(self,exp_temp=False):
        ...
        #Added double power law for DM Tau (KMF: July 2022)
        if len(self.qq)>1:
            #Rbreak = 200.
            if self.qq[2]==1:
                #Double-power law
                tmid = self.tmid0*(rcf/(150*Disk.AU))**self.qq[0]
                tatm = self.tatm0*(rcf/(150*Disk.AU))**self.qq[0]
                tmid[rcf>self.Rbreak*Disk.AU] = self.tmid0*(self.Rbreak/150)**(self.qq[0]-self.qq[1])*(rcf[rcf>self.Rbreak*Disk.AU]/(150*Disk.AU))**self.qq[1]
                tatm[rcf>self.Rbreak*Disk.AU] = self.tatm0*(self.Rbreak/150)**(self.qq[0]-self.qq[1])*(rcf[rcf>self.Rbreak*Disk.AU]/(150*Disk.AU))**self.qq[1]
            elif self.qq[2]==2:
                #Jump in temperature
                tmid = self.tmid0*(rcf/(150*Disk.AU))**self.qq[0]
                tatm = self.tatm0*(rcf/(150*Disk.AU))**self.qq[0]
                tmid[rcf>self.Rbreak*Disk.AU] *= self.qq[1]
                #tatm[rcf>200*Disk.AU] *= self.qq[1]
            elif self.qq[2]==3:
                #separate power laws for surface layers and the midplane
                tmid = self.tmid0*(rcf/(150*Disk.AU))**self.qq[0]
                tatm = self.tatm0*(rcf/(150*Disk.AU))**self.qq[1]
            elif self.qq[2]==4:
                #separate power laws for surface layer and the midplane, and a jump in temperature
                tmid = self.tmid0*(rcf/(150*Disk.AU))**self.qq[0]
                tatm = self.tatm0*(rcf/(150*Disk.AU))**self.qq[1]
                tmid[rcf>self.Rbreak*Disk.AU] *= self.qq[3]
            else:
                #standard model
                tmid = self.tmid0*(rcf/(150*Disk.AU))**self.qq[0]
                tatm = self.tatm0*(rcf/(150*Disk.AU))**self.qq[0]
        ...
        Omg = np.sqrt(np.abs((dPdr/(rcf*self.rho0)+Disk.G*self.Mstar/(rcf**2+zcf**2)**1.5)))
    ...
    def set_rt_grid(self,vcs=True):
        ...
        self.add_mol_ring(self.Rabund[0]/Disk.AU,self.Rabund[1]/Disk.AU,self.sigbound[0]/Disk.sc,self.sigbound[1]/Disk.sc,self.N2H,initialize=True)

        if np.size(self.N2H)>1:
            #gaussian rings
            self.Xmol = self.N2H[0]*np.exp(-(self.Rabund[0]-tr)**2/(2*self.Rabund[3]**2))+self.N2H[1]*np.exp(-(self.Rabund[1]-tr)**2/(2*self.Rabund[4]**2))+self.N2H[2]*np.exp(-(self.Rabund[2]-tr)**2/(2*self.Rabund[5]**2))

        #Freeze-out
        zap = (tT>self.Tco)
        if zap.sum() > 0:
            self.Xmol[zap] = 1./10**(8)*self.Xmol[zap]
        zup = (tT < self.TN2)
        if zup.sum() > 0:
            self.Xmol[zup] = 1./10**(8)*self.Xmol[zup]
        ...
        #tdBV = np.sqrt(2.*Disk.kB/(Disk.Da*Disk.mN2H)*tT+self.vturb**2)
        if vcs:
            tdBV = np.sqrt((1+(self.vturb/Disk.kms)**2.)*(2.*Disk.kB/(Disk.Da*Disk.mN2H)*tT)) #vturb proportional to cs
            #self.r = tr
            #self.Z = tdiskZ
            #vt = self.doppler()*np.sqrt(Disk.Da*Disk.mN2H/(Disk.Da*Disk.mu))
            #tdBV = np.sqrt((1+vt**2.)*(2.*Disk.kB/(Disk.Da*Disk.mN2H)*tT))
        else:
            tdBV = np.sqrt(2.*Disk.kB/(Disk.Da*Disk.mN2H)*tT+self.vturb**2)
    ...
    def set_line(self,line='n2h',vcs=True):
        self.line = line
        try:
            if line.lower()[:2]=='co':
                self.m_mol = 12.011+15.999
            elif line.lower()[:3]=='n2h':
                self.m_mol = 29.0
                ...
                raise ValueError('Choose a known molecule [CO, N2H+ C18O, 13CO, HCO, HCO+, HCN, CS, DCO+] for the line parameter')
            #assume it is DCO+
    ...
    def plot_structure(self,sound_speed=False,beta=None,dust=False,rmax=500,zmax=150):
        ...
        cs3 = plt.contour(self.r[0,:,:]/Disk.AU,self.Z[0,:,:]/Disk.AU,np.log10(np.abs(self.sig_col[0,:,:])),(-2,-1),linestyles=':',linewidths=3,colors='k')
        ax = plt.gca()
        ...
        for tick in ax.yaxis.get_major_ticks():
            ...
            tick.label1.set_fontweight('bold')
        if sound_speed:
            cs = self.r*self.Omg#np.sqrt(2*self.kB/(self.Da*self.mN2H)*self.T)
            ...
            manual_locations=[(150,80),(250,100),(350,120),(450,30)]
            cs3 = plt.contour(self.r[0,:,:]/Disk.AU,self.Z[0,:,:]/Disk.AU,self.T[0,:,:],(10,15,20,25),colors='k',linestyles='--')
