...
def total_model(disk,imres=0.0325,distance=144.5,chanmin=-2.48,nchans=int(20),chanstep=0.26,flipme=True,Jnum=2,freq0=279.51170100,xnpix=576,vsys=6.06,PA=154.8,offs=[-0.0285,-0.01303],modfile='testpy_alma',abund=1.,obsv=obsv,wind=False,isgas=True,includeDust=False,extra=0,bin=1,hanning = False):
...
'''
    ...
    cube=np.zeros((int(disk.nphi),int(disk.nr),int(nchans)))
    cube2=np.zeros((int(disk.nphi),int(disk.nr),int(disk.nz),int(nchans))) #tau
    cube3 = np.zeros((int(disk.nphi),int(disk.nr),int(disk.nz), int(nchans))) #tau_dust
    X = disk.X
    Y = disk.Y
    ...
    # approximation for partition function
        try:
            #The code recognizes 13CO(2-1), C18O(2-1), DCO+(3-2), HCO+(4-3), HCN(4-3), CO(3-2), CS(7-6), CO(1-0), CO(2-1), CO(6-5), DCO+(5-4), DCO+(4-3), C18O(3-2), C18O(1-0), N2H+(3-2)
            if Jnum == 1 and np.abs(freq0-220.398677) < .1:
                ...
                moldat = mol_dat(file='dcoplus.dat')
            elif Jnum==2 and np.abs(freq0-279.51170100)<0.1:
                moldat = mol_dat(file='n2h.dat')
            else:
                raise ValueError('Make sure that Jnum and freq0 match one of: 13CO(2-1), C18O(2-1), DCO+(3-2), HCO+(4-3), HCN(4-3), CO(3-2), CS(7-6), CO(1-0), CO(2-1), CO(6-5), DCO+(5-4), DCO+(4-3), C18O(3-2), C18O(1-0), N2H+(3-2)')
        except:
            raise
        gl = 2.*obs[0]+1
    ...
    for i in range(int(dchans)):
        ...
        #print('Finished channel %i / %i' % (i+1,nchans))
            ...
            cube2[:,:,:,i] = tau_dust
    if flipme:
        cube[:,:,int(dchans):] = cube[:,:,-int((dchans+1)):-int((nchans+1)):-1]
        cube2[:,:,:,int(dchans):] = cube2[:,:,:,-int((dchans+1)):-int((nchans+1)):-1]
        cube3[:,:,:,int(dchans):] = cube3[:,:,:,-int((dchans+1)):-int((nchans+1)):-1]
...
def plot_tau1(disk,tau,tau_dust):
    ...
    cs2 = plt.contour(disk.r[iphi,:,:]/disk.AU,disk.Z[iphi,:,:]/disk.AU,np.log10((disk.rhoG)[iphi,:,:]),np.arange(-8,0.,0.1))
    #cs2 = plt.contour(disk.r[iphi,:,:]/disk.AU,disk.Z[iphi,:,:]/disk.AU,np.log10((disk.rhoH2)[iphi,:,:]),np.arange(0,11,0.1))
    cs3 = plt.contour(disk.r[iphi,:,:]/disk.AU,disk.Z[iphi,:,:]/disk.AU,np.log10(np.abs(disk.sig_col[0,:,:])),(-2,-1),linestyles=':',linewidths=3,colors='k')
    manual_locations=[(250,120),(350,120),(600,50),(140,120)]
    cs3 = plt.contour(disk.r[iphi,:,:]/disk.AU,disk.Z[iphi,:,:]/disk.AU,disk.T[iphi,:,:],(10,14,19,25),colors='k',linestyles='-')
    cs = plt.contour(disk.r[iphi,:,:]/disk.AU,disk.Z[iphi,:,:]/disk.AU,tau[iphi,:,:],(1,),colors='b',linestyles='-.',linewidths=3)
    cs_dust = plt.contour(disk.r[iphi,:,:]/disk.AU,disk.Z[iphi,:,:]/disk.AU,tau_dust[iphi,:,:],(1,),colors='b',linestyles ='-.',linewidths=3)
    plt.clabel(cs3,fmt='%1i', manual=manual_locations)
    plt.plot(disk.rf/disk.AU,disk.calcH()/disk.AU,color='m',linestyle='--')
    #plt.clabel(cs3,fmt='%1i',manual=manual_locations)
    ...
    cs2 = plt.contour(-disk.r[iphi,:,:]/disk.AU,disk.Z[iphi,:,:]/disk.AU,np.log10((disk.rhoG)[iphi,:,:]),np.arange(-8,0.,0.1)) #mol number density
    ...
    cs3 = plt.contour(-disk.r[iphi,:,:]/disk.AU,disk.Z[iphi,:,:]/disk.AU,np.log10(np.abs(disk.sig_col[0,:,:])),(-2,-1),linestyles=':',linewidths=3,colors='k')
    manual_locations=[(-590,50),(-375,120),(-260,120),(-140,120)]
    cs3 = plt.contour(-disk.r[iphi,:,:]/disk.AU,disk.Z[iphi,:,:]/disk.AU,disk.T[iphi,:,:],(10,14,19,25),colors='k',linestyles='-')
    cs = plt.contour(-disk.r[iphi,:,:]/disk.AU,disk.Z[iphi,:,:]/disk.AU,tau[iphi,:,:],(1,),colors='b',linestyles='-.',linewidths=3)
    cs_dust = plt.contour(-disk.r[iphi,:,:]/disk.AU,disk.Z[iphi,:,:]/disk.AU,tau_dust[iphi,:,:],(1,),colors='b',linestyles ='-.',linewidths=3)
    plt.plot(-disk.rf/disk.AU,disk.calcH()/disk.AU,color='m',linestyle='--')
...
def flux_range(disk,cube3,r0,height=True):
    ...
    for iv in range(nchans):
        ...
        if height:
            ...
            ztau_all = np.concatenate((ztau_all,z[w]))
        else:
            #ztau_all = np.concatenate((ztau_all,cube3v[w]*disk.T[w]))
            ztau_all = np.concatenate((ztau_all,disk.T[w]))
    ...
    wuse = flux_all>.05*flux_all.max()#.05
    #w = np.argsort(ztau_all[wuse])

    #plt.figure()
    #plt.subplot(121)
    #plt.hist(ztau_all[wuse],10000,histtype='step')
    #plt.axvline(ztau_all[wuse][w][int(.25*len(w))])
    #plt.axvline(ztau_all[wuse][w][int(.75*len(w))])
    wq = DescrStatsW(data=ztau_all[wuse],weights=flux_all[wuse])
    quant = wq.quantile(probs=np.array([.05,.16,.25,.5,.75,.84,.95]),return_pandas=False)
    #plt.axvline(quant[0],color='r')
    #plt.axvline(quant[2],color='r')
    #plt.subplot(122)
    #plt.plot(flux_all,ztau_all,'.k')
    #plt.axhline(ztau_all[wuse][w][int(.25*len(w))])
    #plt.axhline(ztau_all[wuse][w][int(.75*len(w))])
    #plt.axhline(quant[0],color='r')
    #plt.axhline(quant[2],color='r')
    #plt.axvline(.05*flux_all.max())
    if height:
        #print('R, Z(25%), Z(75%), total flux: ',r0/disk.AU,ztau_all[wuse][w][int(.25*len(w))],ztau_all[wuse][w][int(.75*len(w))],np.sum(flux_all[wuse]))
        print('R, Z(5%), Z(16%), Z(25%), Z(50%), Z(75%), Z(84%), Z(95%), total flux: {:0.1f}, {:0.2f}, {:0.2f}, {:0.2f}, {:0.2f}, {:0.2f}, {:0.2f}, {:0.2f}, {:0.3e}'.format(r0/disk.AU,quant[0],quant[1],quant[2],quant[3],quant[4],quant[5],quant[6],np.sum(flux_all[wuse])))
        #print(quant[0],quant[2])
    else:
        #print('R, T(25%), T(75%): ',r0/disk.AU,ztau_all[wuse][w][int(.25*len(w))],ztau_all[wuse][w][int(.75*len(w))])
        print('R, T(5%), T(16%), T(25%), T(50%), T(75%), T(84%), T(95%), total flux: {:0.1f}, {:0.2f}, {:0.2f}, {:0.2f}, {:0.2f}, {:0.2f}, {:0.2f}, {:0.2f}, {:0.3e}'.format(r0/disk.AU,quant[0],quant[1],quant[2],quant[3],quant[4],quant[5],quant[6],np.sum(flux_all[wuse])))
