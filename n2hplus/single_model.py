...
##############################################################################
def make_model_vis(datfile='alma.n2hdata',modfile='testpy_alma_n2h',isgas=True,freq0= 279.51170100):
    if isgas:
...
def compare_vis(datfile='alma.n2hdata',modfile='testpy_alma_n2h',new_weight=[1,],systematic=False,isgas=True,plot_resid=False):
    '''Calculate the raw chi-squared based on the difference between the model and data visibilities.

    :param datfile: (default = 'alma.n2hdata')
     The base name for the data file. The code reads in the visibilities from datfile+'.vis.fits'

     :param modfile" (default='model/testpy_alma_n2h')
...
def compare_vis_galario(datfile='alma.n2hdata',modfile='testpy_alma_n2h',new_weight=[1,],systematic=False,isgas=True,plot_resid=False):
    '''Calculate the raw chi-squared based on the difference between the model and data visibilities.

    :param datfile: (default = 'alma.n2hdata')
     The base name for the data file. The code reads in the visibilities from datfile+'.vis.fits'

     :param modfile" (default='testpy_alma_n2h')
     The base name for the model file. The code reads in the visibilities from modfile+'.model.vis.fits'
...
'''
    ...
    if isgas:
        ...
        for i in range(real_obj.shape[1]):
            #print(model.shape,u_obj.shape,real_obj.shape)
...
def lnlike(p,highres=False,massprior=False,cleanup=True,systematic=False,line='n2h32',vcs=True,exp_temp=False,add_ring=False,use_galario=True):
    ...
    start=time.time()
    all_params = {'q':p[0],#(-.371,p[1],2),#p[0], #q
    ...
    'Rc':10**(2.444), #Rc
    'incl':-36., #inclination, degrees
    'Mstar':0.54, #solar masses
    'Xn2h':10**(p[1]), #Abundance
    ...
    if all_params['Mdisk'] <0 or all_params['Mdisk']>all_params['Mstar'] or all_params['Rin']<0 or all_params['Rin']>all_params['Rout'] or all_params['Rout']<0 or all_params['Rc']<0 or all_params['Mstar']<0 or all_params['vturb']<0 or all_params['Zq0']<0 or all_params['Tmid0']<0 or all_params['Tmid0']>all_params['Tatm0'] or all_params['Tatm0']<0 or all_params['Zabund'][0]<0 or all_params['Zabund'][1]<0 or all_params['Zabund'][1]<all_params['Zabund'][0] or all_params['Rabund'][0]<0 or all_params['Rabund'][0]<all_params['Rin'] or all_params['Rabund'][0]>all_params['Rabund'][1] or all_params['Rabund'][1]<0 or all_params['Rabund'][1]>all_params['Rout']:
        ...
        if line.lower() == 'n2h32':
            ...
            xoff = all_params['xoff']
            yoff = all_params['yoff']
            resolution = 0.0325
            ...
            disk_structure.add_mol_ring(p[2],p[2]+50,3,100,10**(p[3])) #3,100 #.01, .79
