...
def lnlike(p,massprior=False,cleanup=True,systematic=False,line='n2h32',vcs=True,exp_temp=False,add_ring=False):
    ...
    p (default = None): Required list of parameter values to generate the model. p = [q,abund,Rring,abund2,turbulence, x-offset, y-offset, Rring_width]. For example, p=((-.371,-.371,0),-11.1,325,-100,0.,0.,0.50.)
    
    massprior (default = False): Include a prior on the disk mass. NOTE: This functionality is not used in analyzing the N2H+ data, but is included here as an example of how to include a prior on one of the input parameters.
    ...
    line (default = 'n2h32'): The molecule to model. For now, only N2H+ is implemented.
