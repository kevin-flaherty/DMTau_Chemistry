...
class Disk:
    ...
    mDCO    = mCO+2.014               # - DCO molecular weight
    ...
    def __init__(self,params=[-0.4,0.04,1.,1.,1000.,10**(2.823),-36,0.54,10**(-11),0.25,70.,15.,24.68,
                 [.79,1000],[10.,1000],-1],obs=[180,131,300,170],rtg=True,vcs=True,
                 exp_temp=False,line='dco',ring=None):
    ...
    def set_params(self,params):
        ...
        self.Xdco = params[8]                # - CO gas fraction
    ...
    def set_structure(self,exp_temp=False):
        ...
        if len(self.qq)>1:
            ...
            if self.qq[2]==1:
                ...
                tmid[rcf>Rbreak*Disk.AU] = self.tmid0*(Rbreak/150)**(self.qq[0]-self.qq[1])*(rcf[rcf>Rbreak*Disk.AU]/(150*Disk.AU))**self.qq[1]
                tatm[rcf>Rbreak*Disk.AU] = self.tatm0*(Rbreak/150)**(self.qq[0]-self.qq[1])*(rcf[rcf>Rbreak*Disk.AU]/(150*Disk.AU))**self.qq[1]
            elif self.qq[2]==2:
                #Jump in temperature
                tmid = self.tmid0*(rcf/(150*Disk.AU))**self.qq[0]
                tatm = self.tatm0*(rcf/(150*Disk.AU))**self.qq[0]
                tmid[rcf>Rbreak*Disk.AU] *= self.qq[1]
                ...
                tmid = self.tmid0*(rcf/(150*Disk.AU))**self.qq[0]
                tatm = self.tatm0*(rcf/(150*Disk.AU))**self.qq[1]
                tmid[rcf>Rbreak*Disk.AU] *= self.qq[3]
            else:
                #standard model
                tmid = self.tmid0*(rcf/(150*Disk.AU))**self.qq[0]
                tatm = self.tatm0*(rcf/(150*Disk.AU))**self.qq[0]
    ...
    def set_rt_grid(self,vcs=True):
        ...
        #self.add_mol_ring(self.Rabund[0]/Disk.AU,self.Rabund[1]/Disk.AU,self.sigbound[0]/Disk.sc,self.sigbound[1]/Disk.sc,self.Xdco,initialize=True) ##Commented out (KMF)

        if np.size(self.Xdco)>1:
            #gaussian rings
            #self.Xmol = self.Xdco[0]*np.exp(-(self.Rabund[0]-tr)**2/(2*self.Rabund[3]**2))+self.Xdco[1]*np.exp(-(self.Rabund[1]-tr)**2/(2*self.Rabund[4]**2))+self.Xdco[2]*np.exp(-(self.Rabund[2]-tr)**2/(2*self.Rabund[5]**2))
            self.Xmol = np.zeros(np.shape(self.r))+1e-34
            zup = (tT<70.) & (tT>30.)
            if zup.sum() > 0:
            ...
            #zup = (tT<18.)
            #if zup.sum() > 0:
            #    self.Xmol[zup] += self.Xdco[2]

        #Freeze-out
        #zap = (tT>70.)
        #if zap.sum() > 0:
        #    self.Xmol[zap] = 1/10**(5)*self.Xmol[zap]



        #zep = (tT<30.)
       # if zep.sum() > 0:
            #self.Xmol[zep] = 1/10**(5)*self.Xmol[zep]

        #zep = (tT>30.)
        #if zep.sum() > 0:
        #    self.Xmol[zep] = 1/2*self.Xmol[zep]

        #zop = (tT<18.)
        #if zop.sum() > 0:
        #    self.Xmol[zop] = 1/10**(5)*self.Xmol[zop]


        #zup = (tT<30.)
        #if zup.sum() > 0:
            #self.Xmol[zup] = 1/10**(5)*self.Xmol[zup]
        ...
        #tdBV = np.sqrt(2.*Disk.kB/(Disk.Da*Disk.mDCO)*tT+self.vturb**2)
        if vcs:
            tdBV = np.sqrt((1+(self.vturb/Disk.kms)**2.)*(2.*Disk.kB/(Disk.Da*Disk.mDCO)*tT)) #vturb proportional to cs
            #self.r = tr
            #self.Z = tdiskZ
            #vt = self.doppler()*np.sqrt(Disk.Da*Disk.mDCO/(Disk.Da*Disk.mu))
            #tdBV = np.sqrt((1+vt**2.)*(2.*Disk.kB/(Disk.Da*Disk.mDCO)*tT))
        else:
            tdBV = np.sqrt(2.*Disk.kB/(Disk.Da*Disk.mDCO)*tT+self.vturb**2)
    ...
    def set_line(self,line='dco',vcs=True):
        self.line = line
        try:
            if line.lower()[:2]=='co':
    ...
    def add_mol_ring(self,Rin,Rout,Sig0,Sig1,abund,alpha=0,initialize=False,just_frozen=False):
        ...
        if just_frozen:
            ...
            add_mol = (self.sig_col*Disk.Hnuctog/Disk.m0>Sig0*Disk.sc) & (self.sig_col*Disk.Hnuctog/Disk.m0<Sig1*Disk.sc) & (self.r>Rin*Disk.AU) & (self.r<Rout*Disk.AU) & (self.Z<0)
        if add_mol.sum()>0:
    ...
    def plot_structure(self,sound_speed=False,beta=None,dust=False,rmax=500,zmax=150):
        ...
        for tick in ax.yaxis.get_major_ticks():
            ...
            tick.label1.set_fontweight('bold')
        if sound_speed:
            cs = self.r*self.Omg#np.sqrt(2*self.kB/(self.Da*self.mDCO)*self.T)
